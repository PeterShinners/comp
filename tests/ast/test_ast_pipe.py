...
- Pipelines now use square brackets: [data |op] or [|op]
- All assignments must be within {} braces

PARSER EXPECTATIONS:
- asttest.parse_value("{x = data}", comp.ast.Structure) → Structure with StructAssign
...
def test_pipeline_in_struct():
    ...
    struct = asttest.parse_value("{x = [|process]}", comp.ast.Structure)
    asttest.roundtrip(struct)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
...
def test_pipeline_with_seed():
    ...
    struct = asttest.parse_value("{x = [data |process]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
...
def test_multi_stage_pipeline():
    ...
    struct = asttest.parse_value("{x = [data |validate |transform]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
...
def test_pipeline_with_scope():
    ...
    struct = asttest.parse_value("{x = [$in |process]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    unparsed = value.unparse()
...
def test_pipeline_with_numbers():
    ...
    struct = asttest.parse_value("{x = [42 |double]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
...
def test_pipeline_with_strings():
    ...
    struct = asttest.parse_value('{x = ["hello" |upper]}', comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
...
def test_pipeline_with_field_access():
    ...
    struct = asttest.parse_value("{x = [obj.field |process]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
...
def test_pipeline_failure_operator():
    ...
    struct = asttest.parse_value("{x = [data |? fallback]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "|?" in unparsed
...
def test_pipeline_wrench_operator():
    ...
    struct = asttest.parse_value("{x = [data |-| modifier]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "|-|" in unparsed
...
def test_chained_wrench_operator():
    ...
    struct = asttest.parse_value("{x = [data |-| optimize |-| debug]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "|-|" in unparsed
...
def test_wrench_with_field_access():
    ...
    struct = asttest.parse_value("{x = [obj.field |-| modifier]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
...
def test_combined_pipeline_failure_and_wrench():
    ...
    struct = asttest.parse_value("{x = [data |? fallback |-| debug]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "data" in unparsed
...
def test_pipeline_with_regular_and_failure():
    ...
    struct = asttest.parse_value("{x = [data |process |? fallback]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "data" in unparsed
...
def test_pipeline_with_struct_literal():
    ...
    struct = asttest.parse_value("{x = [data |{field = value}]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "data" in unparsed
...
def test_pipeline_with_block_literal():
    ...
    struct = asttest.parse_value("{x = :{expression}}", comp.ast.Structure)
...
def test_multiple_assignments_with_pipelines():
    ...
    struct = asttest.parse_value("{x = [data |process], y = [$in |validate]}", comp.ast.Structure)
...
def test_nested_structures_with_pipelines():
    ...
    struct = asttest.parse_value("{outer = {inner = [data |process]}}", comp.ast.Structure)
    unparsed = struct.unparse()
...
def test_long_pipeline_chain():
    ...
    struct = asttest.parse_value("{x = [data |func1 |func2 |func3 |func4 |func5]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
    assert "data" in unparsed
...
def test_pipeline_roundtrip():
    ...
    struct1 = asttest.parse_value(original, comp.ast.Structure)
    unparsed = struct1.unparse()
    struct2 = asttest.parse_value(unparsed, comp.ast.Structure)

    # Both should have pipelines as values
    key1, value1 = asttest.structure_field(struct1, 0, comp.ast.Pipeline)
    key2, value2 = asttest.structure_field(struct2, 0, comp.ast.Pipeline)
    asttest.roundtrip(struct1)


def test_empty_pipeline():
    """Test assignment without pipeline (just a value)."""
    struct = asttest.parse_value("{x = 42}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Number)
    assert key == "x"
    asttest.roundtrip(struct)
...
def test_pipeline_with_tag():
    ...
    struct = asttest.parse_value("{x = [#mytag |process]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
...
def test_pipeline_with_parenthesized_expression():
    ...
    struct = asttest.parse_value("{x = [(a + b) |double]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "x"
    unparsed = value.unparse()
...
def test_complex_pipeline_expression():
    ...
    struct = asttest.parse_value("{result = [$in |filter |? backup |-| progressbar]}", comp.ast.Structure)
    key, value = asttest.structure_field(struct, 0, comp.ast.Pipeline)
    assert key == "result"
    unparsed = value.unparse()
    assert "$in" in unparsed
