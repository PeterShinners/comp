...
@pytest.mark.skip(reason="Reference literals not implemented yet")
def test_parse_simple_tags():
    """Simple tag references without hierarchy."""
    result = comp.parse("#true")
    assert isinstance(result, comp.TagLiteral)
...
def test_parse_simple_shapes():
    ...
    assert isinstance(result, comp.ShapeLiteral)
...
def test_parse_simple_functions():
    ...
    assert isinstance(result, comp.FunctionLiteral)
...
def test_unified_pattern_consistency():
    ...
    assert isinstance(tag, comp.TagLiteral)
    assert isinstance(shape, comp.ShapeLiteral)
    assert isinstance(func, comp.FunctionLiteral)
...
def test_invalid_reference_formats():
    ...
    for invalid in invalid_references:
        with pytest.raises(comp.ParseError, match="Invalid reference"):
            comp.parse(invalid)
...
def test_invalid_hierarchy_formats():
    ...
    for invalid in invalid_hierarchies:
        with pytest.raises(comp.ParseError, match="Invalid hierarchy"):
            comp.parse(invalid)
...
def test_invalid_module_formats():
    ...
    for invalid in invalid_modules:
        with pytest.raises(comp.ParseError, match="Invalid module reference"):
            comp.parse(invalid)
...
def test_integration_with_existing_literals():
    ...
    assert isinstance(tag_result, comp.TagLiteral)
    assert tag_result.name == "active"

    shape_result = comp.parse("~num")
    assert isinstance(shape_result, comp.ShapeLiteral)
    assert shape_result.name == "num"

    func_result = comp.parse("|connect")
    assert isinstance(func_result, comp.FunctionLiteral)
