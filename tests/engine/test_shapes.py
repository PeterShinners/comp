...
from comp.engine.ast._shape import ShapeDef, ShapeFieldDef, ShapeRef, ShapeUnion


def test_entity_base_class_shapes():
    """Verify ShapeField and ShapeDefinition work with module system."""
    module = comp.Module()

    # Define a simple shape
    from comp.engine._module import ShapeField
    fields = [
        ShapeField(name="x", shape=None),  # ~num would go here
        ShapeField(name="y", shape=None),
    ]
...
def test_module_shape_storage():
    ...
    from comp.engine._module import ShapeField
    fields = [ShapeField(name="name", shape=None)]
...
def test_simple_shape_definition():
    ...
    shape_def = ShapeDef(
        path=["point"],
        fields=[
            ShapeFieldDef(name="x"),
            ShapeFieldDef(name="y"),
        ]
    )
...
def test_positional_shape_fields():
    ...
    shape_def = ShapeDef(
        path=["pair"],
        fields=[
            ShapeFieldDef(),  # First positional
            ShapeFieldDef(),  # Second positional
        ]
    )
...
def test_shape_reference():
    ...
    module = comp.Module()
    from comp.engine._module import ShapeField
    fields = [ShapeField(name="x"), ShapeField(name="y")]
...
def test_shape_reference_not_found():
    ...
    shape_ref = ShapeRef(["nonexistent"])
...
def test_spread_expansion():
    ...
    point_3d = ShapeDef(
        path=["point-3d"],
        fields=[
            ShapeFieldDef(is_spread=True, shape_ref=ShapeRef(["point-2d"])),
            ShapeFieldDef(name="z"),
        ]
    )
    ...
    # (ShapeField doesn't have is_spread attribute - only AST nodes do)
...
def test_mixed_named_positional_fields():
    ...
    shape_def = ShapeDef(
        path=["labeled"],
        fields=[
            ShapeFieldDef(),              # Positional
            ShapeFieldDef(name="label"),  # Named
        ]
    )
