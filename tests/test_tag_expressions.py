...
class TestTagExpressions:
    ...
    def test_tag_arithmetic_addition(self):
        ...
        # First body kid should be a comp.ast.BinaryOp
        assert len(tag.body_kids) == 1
        assert isinstance(tag.body_kids[0], comp.ast.BinaryOp)
    ...
    def test_tag_bitshift_left(self):
        ...
        # First body kid should be a comp.ast.BinaryOp with << operator
        assert isinstance(tag.body_kids[0], comp.ast.BinaryOp)
        assert tag.body_kids[0].op == "<<"
    ...
    def test_tag_unary_plus(self):
        ...
        tag = result.kids[0]
        assert isinstance(tag.body_kids[0], comp.ast.UnaryOp)
        assert tag.body_kids[0].op == "+"
    ...
    def test_tag_expression_with_children(self):
        ...
        assert isinstance(tag.body_kids[0], comp.ast.BinaryOp)  # 1<<0
        assert isinstance(tag.body_kids[1], comp.ast.TagChild)  # #active (comp.ast.TagChild, not comp.ast.TagDefinition)
        assert isinstance(tag.body_kids[2], comp.ast.TagChild)  # #inactive
    ...
    def test_tag_expression_with_generator_and_children(self):
        ...
        tag = result.kids[0]
        assert isinstance(tag, comp.ast.TagDefinition)
        assert not isinstance(tag.generator, comp.ast.Placeholder)
...
"""
        ...
        # Each child should have a bit shift expression (comp.ast.TagChild nodes)
        for i, child in enumerate(tag.body_kids):
            ...
            assert isinstance(child.body_kids[0].left, comp.ast.Number)
            assert child.body_kids[0].left.value == 1
            assert isinstance(child.body_kids[0].right, comp.ast.Number)
