...
- `^` - Most commonly used to reference values passed as arguments to the
function. It actually represents a combined representation of the `$arg`,
`$ctx`, and `$mod` scopes together. This scope cannot be overwritten.
...
supplied to the function. These values have been morphed to match the shape
of the function's argument definition.

Most code will reference arguments through the `^` scope, which allows fallback
arguments to come from `$ctx` and `$mod`. This is an important concept that
allows functions to coordinate how they will run without needing to pass
explicit bundles of arguments along function chains.

While a block is being executed, this scope still refers to the arguments
the block was originally defined in.

This scope cannot be overwritten.

```comp
!func |greet ^{name ~str = "World"} = {
    "Hello, ^name!"  ; Uses argument or falls back to context or module
}
```
