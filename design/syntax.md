...
- `$var` - Function-local variables that only exist within the current function.

**Bare Identifiers**

Bare identifiers (tokens without scope prefix or dots) automatically become string literals:

```comp
; These create structures with string values
colors = {red green blue}        ; Same as {"red" "green" "blue"}
config = {theme=dark mode=auto}  ; Fields "theme" and "mode" with string values

; Dotted identifiers remain as field references
user-name = data.name            ; Field access on 'data'
width = $in.width                ; Explicit scope access
```

This automatic conversion simplifies structure literals while requiring explicit scope
prefixes (`$var.`, `$arg.`, etc.) for all field access. No implicit local scope lookups.
...
```comp
!func |build-user arg ~{name ~str age ~num} = {
...
The `$var` scope is used for function-local variables. When the function completes
...
!func |example = {
    ...
    result = data |process :{
        $var.counter = $var.counter + 1  ; Modifies function-level counter
        value = $var.counter * 2
    }
...
The `$arg` scope contains the arguments supplied to the function and is constructed
from three sources with defined precedence. This scope cannot be overwritten.

When a function is invoked, the `$arg` scope is built by merging:
1. Direct arguments passed to the function (highest priority)
2. Matching fields from `$ctx` (context scope)
3. Matching fields from `$mod` (module scope)
4. Default values from the argument shape

Only fields defined in the function's argument shape are included. This automatic
filtering prevents accidental access to unexpected context or module fields.

```comp
; Set up context and module fields
$ctx.port = 8080
$mod.host = "default.com"

!func |connect arg ~{host ~str port ~num timeout ~num = 30} = {
    ; $arg is built from: {host="default.com" port=8080 timeout=30}
    ; - host from $mod (no override)
    ; - port from $ctx  
...
; Graphics operations
!func |graphics.translate ~{point ~graphics.point} arg ~{offset ~graphics.point} = {
    ; Translation logic
}
```
