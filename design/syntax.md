...
When a field is referenced with `.`, the language searches the shared namespaces in order until finding a match. Field assignments always target `$out`, creating or overriding fields in the output structure. This separation between input and output makes data flow explicit and prevents accidental mutations.

```comp
func process pipeline{data} args{port} = {
    $mod.default_port = 8000
    $ctx.override_port = 8080
    
    server_port = $arg.port          # Uses parameter directly
    fallback = $mod.default_port     # From module namespace
    final = $ctx.override_port       # From context
    computed = .port                 # Searches shared namespaces
}
```
...
```comp
func public_api pipeline{data} args{} = {
    $in | validate | internal_process&
}

func internal_process& pipeline{data} args{} = {
...
func configure pipeline{} args{options} = {
