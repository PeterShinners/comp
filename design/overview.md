...
when referenced and support flexible definition styles for different use cases.
...
; Flat style definition - equivalent result
!tag #status.error.timeout = -100
!tag #status.error.network = -200

; References use reversed hierarchy: specific.parent.root
state = #active
error = #timeout.error.status
short = #timeout               ; When unambiguous
```
...
`~rectangle/geometry` or `|sum/math`. Tags follow this same rule, with their
hierarchy listed from specific to general and module namespace after `/`.
`#success.status/http` references the `success` tag from the `status` hierarchy
in the `http` module.

A module's namespace is declaratively defined, and clearly known and understood
before anything is executed. Name errors or ambiguities are immediate build
errors.
...
can be identified as structures with a leading `.` dot prefix. For convenience,
functions expecting block arguments also accept simple values, which are
automatically wrapped in blocks.

```comp
; Explicit block syntax
(|if value > 5 .{|prepare-additional-space} .{|handle-small-set})

; Simplified syntax with automatic block wrapping
(|if value > 5 "large" "small")
@style = (|if complete "strikethrough" "normal")

; Complex blocks still use dot prefix
($in |when .{error?} .{
    (error |log)
})
...
Any field reference not using one of these scopes will use the `$out` and
`$in` pipeline scopes. Assignments to fields without a scope generate
fields in the current output structure.

- `@var` - Function-local variables
- `^` - Function argument scope (cascades to `$ctx`, `$mod`)
...
greeting = {name} % "Hello ${name}"  ; Templated string and operator
