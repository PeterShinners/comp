...
class FieldNode(AstNode):
    ...
    to provide the identifier scope via:
        identifier_scope = Value({'current': current_value})
    ...
    identifier scope since it's always first in the chain (it uses
    ...
    def get_current_value(self, engine) -> Value:
        ...
        Raises:
            RuntimeError: If identifier scope is not set properly
        """
        identifier_scope = engine.get_scope('identifier')
        if identifier_scope is None:
            raise RuntimeError(
                f"{self.__class__.__name__} requires identifier scope. "
                f"Parent must set via: with engine.scope_frame(identifier=...)"
            )

        current_key = Value('current')
        if not identifier_scope.is_struct or current_key not in identifier_scope.struct:
            raise RuntimeError("Invalid identifier scope structure")

        return identifier_scope.struct[current_key]
