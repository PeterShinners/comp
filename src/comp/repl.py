...
class ReplContext:
    ...
    Maintains state across multiple evaluations including @local scope for
    user-defined variables, Module for definitions (!tag, !shape, !func, !import),
    last result (_), and Engine instance.

    Attributes:
        engine: (comp.Engine) The evaluation engine
        module: (comp.Module) Module for definitions
        local_scope: (comp.Value) Local variable scope
        last_result: (comp.Value) Last evaluated result
    """
    ...
    def eval_line(self, line):
        ...
        Supports expressions (5 + 3, {x = 10}), definitions (!tag #status,
        !func |foo, !shape ~point), and references to _ (last result)
        and @local variables.
...
def format_value(value):
    ...
    Args:
        value: (comp.Value) Value to format
